...
class ArmCommand(metaclass=Metaclass_ArmCommand):
    ...
    def __init__(self, **kwargs):
        ...
        if 'joint_positions' not in kwargs:
            self.joint_positions = numpy.zeros(6, dtype=numpy.float64)
        else:
            self.joint_positions = numpy.array(kwargs.get('joint_positions'), dtype=numpy.float64)
            assert self.joint_positions.shape == (6, )
    ...
    def __eq__(self, other):
        ...
        if self.header != other.header:
            return False
        if all(self.joint_positions != other.joint_positions):
            return False
    ...
    def joint_positions(self, value):
        if isinstance(value, numpy.ndarray):
            ...
            assert value.size == 6, \
                "The 'joint_positions' numpy.ndarray() must have a size of 6"
        ...
        if __debug__:
            ...
            assert \
                ...
                "The 'joint_positions' field must be a set or sequence with length 6 and each value of type 'float' and each double in [-.000000, .000000]"
