"""
Task 1 Launch File — Object Retrieval pipeline.

Launches:
  1. detect_object_real.py  — perception (object detection)
  2. navigate_to_object.py  — navigation to standoff position + return to origin
  3. task1_pickup.py        — arm pickup (auto_start mode, waits for trigger)
  4. task1_coordinator.py   — orchestrator (state machine)
...
Usage:
    ...
    # Terminal 2: Launch Task 1 pipeline
...
def generate_launch_description():
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            ...
            description='Timeout for navigation to object (seconds)'
        ),
        DeclareLaunchArgument(
        ...
        DeclareLaunchArgument(
            'return_timeout', default_value='120.0',
            description='Timeout for return to origin (seconds)'
        ),
        DeclareLaunchArgument(
        ...
        Node(
            package='tidybot_bringup',
            executable='task1_coordinator.py',
            name='coordinator',
            output='screen',
            parameters=[{
                ...
                'return_timeout': LaunchConfiguration('return_timeout'),
