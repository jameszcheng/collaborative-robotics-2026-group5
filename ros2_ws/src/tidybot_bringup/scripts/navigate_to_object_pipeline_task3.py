...
VERSION = "1.1.0-object"

# Fix 1 (vs pipeline v1): heading threshold tightened 45deg -> 30deg for earlier speed reduction
# Fix 2 (vs pipeline v1): proportional speed ramp in last SLOWDOWN_ZONE metres before goal
...
class NavigateToObject(Node):
    ...
    def go_to_position(self, goal_x: float, goal_y: float, tolerance: float = None) -> bool:
        ...
        # Fix 2: proportional speed ramp in the last SLOWDOWN_ZONE metres.
        # Without this, kp*distance is capped at max_v for most of the approach,
        # then the proportional term suddenly kicks in close to the goal — abrupt
        # and causes overshoot. Ramping smoothly from max_v down to 0.05 m/s
        # in the last 0.15 m gives the robot time to brake before it arrives.
        SLOWDOWN_ZONE = 0.15  # metres
        if distance < SLOWDOWN_ZONE:
            v = max(0.05, self.max_v * (distance / SLOWDOWN_ZONE))
        else:
            v = np.clip(self.kp * distance, 0.0, self.max_v)

        omega = np.clip(2.0 * self.kp * heading_error, -self.max_omega, self.max_omega)

        # Fix 1: tightened from pi/4 (45deg) to pi/6 (30deg) — slows down earlier
        # to prevent curving wide when the robot is still misaligned on approach.
        if abs(heading_error) > np.pi / 6:
            v *= 0.3
