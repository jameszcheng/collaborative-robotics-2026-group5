#!/usr/bin/env python3
"""
task1_coordinator.py — Orchestrates the full object-retrieval pipeline.

Sequences: NLP/manual trigger -> perception -> navigation -> pickup -> return to origin
...
class CoordinatorNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('return_timeout', 120.0)
        ...
        self.return_to_origin_pub = self.create_publisher(Empty, '/coordinator/return_to_origin', 10)
        ...
        self.get_logger().info('Coordinator node started (Task 1: find object and return to origin)')
        self.get_logger().info('Waiting for voice command or manual trigger...')
        self.get_logger().info('  Voice: "find the banana"')
    ...
    def _tick(self):
        ...
        if self.state == State.IDLE:
            ...
            if self._pickup_complete is True:
                self.get_logger().info('Pickup complete! Object grasped. Returning to origin...')
                self._nav_complete = False
                self._set_state(State.RETURNING)
                self.return_to_origin_pub.publish(Empty())
                ...
                self._fail(
                    ...
                    f"Is task1_pickup.py running?"
                )

        elif self.state == State.RETURNING:
            if self._nav_complete:
                self.get_logger().info('Returned to origin! Task complete.')
                self._set_state(State.DONE)
            elif elapsed > self.return_timeout:
                self._fail(
                    f"Return to origin timed out after {self.return_timeout:.0f}s. "
                    f"Is navigate_to_object.py running?"
                )
