#!/usr/bin/env python3
"""
movement_4.py — TidyBot2 Circular Trajectory Tracking + Go-To-Position
v1.0.6
...
class TrajectoryTracker(Node):
    def __init__(self):
        ...
        # --- Print startup info ---
        self.get_logger().info(f'=== movement_4.py  v{VERSION} ===')
    ...
    def go_to_position(self, goal_x: float, goal_y: float, tolerance: float = None) -> bool:
        ...
        if self.robot == 'sim':
            # Many sims for TidyBot use /base/target_pose rather than /cmd_vel.
            pose = Pose2D()
            pose.x = float(ref_x)
            pose.y = float(ref_y)
            pose.theta = 0.0
